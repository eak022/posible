...
const purchaseOrderService = {
    ...
    // รับสินค้าจากข้อมูลการส่งมอบจริง
    receiveStockFromDelivery: async (purchaseOrderId) => {
        try {
            const response = await api.post(`/purchase-orders/${purchaseOrderId}/receive-from-delivery`);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    // อัพเดทข้อมูลการส่งมอบ
    updateDeliveryInfo: async (purchaseOrderId, deliveryData) => {
        try {
            const response = await api.put(`/purchase-orders/${purchaseOrderId}/delivery-info`, deliveryData);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    // อัปเดตใบสั่งซื้อและสร้างล็อตใหม่ (สำหรับแก้ไขข้อมูลการรับสินค้า)
    updatePurchaseOrderAndRecreateLots: async (purchaseOrderId, updateData) => {
        try {
            const response = await api.put(`/purchase-orders/${purchaseOrderId}/update-and-recreate-lots`, updateData);
            return response.data;
        } catch (error) {
            throw error;
        }
