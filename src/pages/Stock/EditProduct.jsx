...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProductData(product); // เก็บข้อมูล product ไว้ใช้ในการตรวจสอบราคา
  ...
  const handleChange = (e) => {
    ...
    // ตรวจสอบราคาขายไม่ให้ต่ำกว่าราคาซื้อ (ใช้ข้อมูลจาก lots)
    ...
    // ใช้ราคาซื้อต่อชิ้นจากล็อตล่าสุด (เรียงตาม receivedDate)
    const activeLots = productData?.lots?.filter(lot => lot.status === 'active') || [];
    const latestLot = activeLots.length > 0 
      ? activeLots.sort((a, b) => new Date(b.receivedDate) - new Date(a.receivedDate))[0]
      : null;
    const latestPurchasePricePerUnit = latestLot ? latestLot.purchasePrice : 0;
    
    setPriceWarning({
      unit: sellingPricePerUnit !== "" && latestPurchasePricePerUnit > 0 && Number(sellingPricePerUnit) < latestPurchasePricePerUnit,
      pack: sellingPricePerPack !== "" && latestPurchasePricePerUnit > 0 && packSize > 0 && Number(sellingPricePerPack) < latestPurchasePricePerUnit * Number(packSize)
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    if (Number(formData.packSize) < 0 || Number(formData.sellingPricePerUnit) < 0 || Number(formData.sellingPricePerPack) < 0) {
    ...
    if (priceWarning.unit || priceWarning.pack) {
      Swal.fire({ icon: 'error', title: 'ราคาขายต้องไม่ต่ำกว่าราคาซื้อต่อชิ้นของล็อตล่าสุด!' });
      setLoading(false);
      return;
    }
                ...
                <div className="space-y-4">
                  ...
                  {(priceWarning.unit || priceWarning.pack) && (
                    <div className="text-xs text-red-500 mt-1">
                      {priceWarning.unit && "ราคาขายต่อชิ้นต้องไม่ต่ำกว่าราคาซื้อต่อชิ้นของล็อตล่าสุด"}
                      {priceWarning.unit && priceWarning.pack && " และ "}
                      {priceWarning.pack && "ราคาขายต่อแพ็คต้องไม่ต่ำกว่าราคาซื้อต่อชิ้นของล็อตล่าสุด × จำนวนชิ้นต่อแพ็ค"}
                    </div>
                  )}
                </div>
                ...
                {formData.productStatuses && formData.productStatuses.length > 0 && (
                  <div className="mb-4">
                   ...
                   <select
                     ...
                     value={formData.productStatuses.length > 0 ? formData.productStatuses[0]._id || formData.productStatuses[0] : ""}
                     onChange={(e) => {
                       const selectedStatus = e.target.value;
                       if (selectedStatus) {
                         setFormData({
                           ...formData,
                           productStatuses: [selectedStatus]
                         });
                       } else {
                         setFormData({
                           ...formData,
                   ...
                   <p className="text-xs text-blue-500 mt-1">
                     * สามารถเลือกได้เฉพาะสถานะ "วางจำหน่าย" และ "เลิกขาย" เท่านั้น
                   </p>
                 </div>
              </div>
            </div>
