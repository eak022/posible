...
const CreatePromotionModal = ({ isOpen, onClose, initialProduct }) => {
  ...
  const [selectedLots, setSelectedLots] = useState([]); // array of lotNumber
  ...
  const handleConfirm = async () => {
    ...
    if (selectedLots.length === 0) {
      Swal.fire({
        icon: "error",
        title: "กรุณาเลือกอย่างน้อย 1 ล็อต",
      });
      return;
    }
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg w-3/4 max-w-2xl">
        ...
        <div className="space-y-4">
         ...
         <div className="flex space-x-4">
          ...
          {selectedProduct && (
            <div className="mt-2">
              <label className="block text-sm mb-1 text-black">เลือกล็อตที่เข้าร่วมโปร (เลือกได้หลายล็อต)</label>
              <div className="max-h-40 overflow-auto border rounded-md p-2 space-y-2">
                {productLots.filter(l => l.status === 'active' && l.quantity > 0 && new Date(l.expirationDate) > new Date()).length === 0 && (
                  <div className="text-sm text-gray-500">สินค้านี้ยังไม่มีล็อต</div>
                )}
                {productLots
                  .filter(l => l.status === 'active' && l.quantity > 0 && new Date(l.expirationDate) > new Date())
                  .map((lot) => (
                  <label key={lot.lotNumber} className="flex items-center gap-2 text-sm">
                    ...
                    />
                    <span>
                      ล็อต {lot.lotNumber} • คงเหลือ {lot.quantity} • หมดอายุ {lot.expirationDate ? new Date(lot.expirationDate).toLocaleDateString() : "-"}
                    </span>
                  </label>
                ))}
              </div>
