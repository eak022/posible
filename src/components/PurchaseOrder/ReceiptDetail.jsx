...
const ReceiptDetail = ({ id, onClose }) => {
  ...
  return (
    <>
      {/* Print Area สำหรับใบรับสินค้า */}
      {showPrintReceipt && (
        <div id="print-receipt-area" ref={printRef} style={{ width: '210mm', minHeight: '297mm', margin: '0 auto', background: '#fff', padding: 24, fontFamily: 'Tahoma, Arial, sans-serif', color: '#222', fontSize: 14, position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', right: 32, top: 24, fontSize: 14, textAlign: 'right', background: '#fff', padding: '2px 8px', borderRadius: 4 }}>
            ...
            วันที่รับ: <b>{purchaseOrder?.createdAt ? new Date(purchaseOrder.createdAt).toLocaleDateString('th-TH') : '-'}</b>
          </div>
        ...
        <div className="bg-white rounded-lg w-full max-w-4xl overflow-y-auto max-h-[80vh]">
          ...
          <div className="flex justify-between items-center p-4 border-b">
            ...
            <div className="flex gap-2 items-center">
              <button
                id="receipt-detail-print-button"
              ...
              <button
                id="receipt-detail-close-button"
                onClick={onClose}
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <div>
                <p className="text-gray-600">วันที่รับสินค้า</p>
              ...
              <table className="w-full">
                <thead className="bg-green-100">
                  <tr>
                    ...
                    <th className="py-3 px-4 text-right">ราคาจริง/หน่วย</th>
                  ...
                  {purchaseOrder?.products?.map((product, index) => (
                    <tr key={index} className="border-b">
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className="text-gray-600">{product.orderedQuantity}</span>
                      </td>
                      <td className="py-3 px-4 text-center">
                        <span className="font-semibold text-green-600">{product.deliveredQuantity || 0}</span>
                      </td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <div>
                          {product.actualPrice ? (
                            <div className="font-semibold text-green-600">{product.actualPrice?.toLocaleString()} บาท</div>
                          ) : (
                        ...
                        <div>
                          {product.actualPrice && product.deliveredQuantity > 0 ? (
                            <div className="font-semibold text-green-600">
                              {((product.actualPrice || 0) * (product.deliveredQuantity || 0)).toLocaleString()} บาท
                            </div>
                          ) : (
                            <div className="text-sm text-gray-500">
                              โดยประมาณ: {((product.estimatedPrice || 0) * (product.deliveredQuantity || 0)).toLocaleString()} บาท
                            </div>
                          )}
                        </div>
                      </td>
      ...
      <style>{`
        @media print {
          ...
          #print-receipt-area { position: absolute !important; left: 0; top: 0; width: 100vw !important; min-height: 100vh !important; background: #fff !important; z-index: 9999; }
        }
      `}</style>
    </>
  );
